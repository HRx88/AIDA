...
while True:
    input("\nPress ENTER to talk (type 'q' then ENTER to quit): ")  #simulate push to talk button
    

    with sr.Microphone(device_index=MIC_INDEX) as source:
        print("Listening... speak normally (up to 30s)")
        audio = recognizer.listen(source, phrase_time_limit=30)     # Records audio from the microphone for 30 seconds or until silence is detected
    ...
    try:
        ...
        if "stop" in text.lower() or "quit" in text.lower():        # Instead of stopping recording mid-speech, we stop the conversation LOGICALLY based on what was said.
