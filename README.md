...
## Project Overview
AIDA is a cloud-native microservices-based system designed to support Persons with Intellectual Disability (PWIDs) in completing and tracking Activities of Daily Living (ADLs) such as self-care routines and household tasks. The system empowers users through gentle AI-powered guidance, smart reminders, and engaging digital interactions, including a conversational voice companion driven by Google Gemini, a virtual pet whose mood evolves with the user's progress, and a gamified rewards mechanism that reinforces consistent task completion. AIDA can be accessed via a standard web browser or deployed as a hands-free Raspberry Pi kiosk for always-on support in residential care environments.

Beyond supporting PWIDs, AIDA provides MINDS staff and volunteers with a dedicated management portal to register and manage client profiles, configure personalized task schedules, upload profile images, and communicate in real time through secure video and emergency calls. The system is built on a modular Docker Compose architecture with seven independently deployable microservices, a host-based voice server for local audio hardware access, and integration with external services including Whereby, Supabase, and Google Cloud. Designed for long-term usability, ethical AI integration, and flexible deployment, AIDA balances powerful assistive technology with a strong commitment to user privacy and dignity.
